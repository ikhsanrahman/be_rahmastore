...
from utilization.decorator import user_token_required
...
class UserRegisterView(viewsets.ViewSet):
    def create(self, request):
        ...
        if not user and serializer.is_valid():
            new_user = MyUser(email=data['email'], username=data['username'], password=data['password'])
...
class UserLogoutView(viewsets.ViewSet):
    ...
    def list(self, request, user_uuid):
        ...
        responses = {}
        user = MyUser.objects.get(id=user_uuid, is_login=True)
...
class UserLoginView(viewsets.ViewSet):
    def create(self, request):
        responses = {}
        data = request.data
        user = MyUser.objects.get(is_login=False, email=data['email'])
...
class GetOneProductView(viewsets.ViewSet):
    ...
    def retrieve(self, request, user_uuid):
        ...
        user = MyUser.objects.filter(id=user_uuid)
...
class AddProductView(viewsets.ViewSet):
    ...
    def create(self, request, user_uuid):
        ...
        user = MyUser.objects.get(id=user_uuid)
